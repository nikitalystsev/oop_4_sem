...
void Matrix<T>::_check_sizes(const Matrix<T> &matrix) const
{
    if (_rows != matrix._rows || _cols != matrix._cols) 
    {
        ...
        throw IncompatibleElements(asctime(local_time), __FILE__, typeid(*this).name(), __LINE__, "Different matrix sizes");
    }
}
...
void Matrix<T>::_check_index(size_t pos, size_t limit) const
{
    if (pos > limit) 
    {
        time_t cur_time = time(NULL);
        auto local_time = localtime(&cur_time);
        throw IndexError(asctime(local_time), __FILE__, typeid(*this).name(), __LINE__, "Index is bigger than sizes");
    }
}
...
SharedPtr<typename Matrix<T>::MatrixRow[]> Matrix<T>::_mem_alloc(size_t rows, size_t cols)
{
    ...
    catch (std::bad_alloc &err) 
    {
        time_t cur_time = time(NULL);
        auto local_time = localtime(&cur_time);
        throw MemoryError(asctime(local_time), __FILE__, typeid(*this).name(), __LINE__, "_allocateMemory function error");
    }
